...
class VEBert(BertPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        if config.classifier == 'att':
            self.ke_query = nn.Linear(config.hidden_size, 1)
            self.ke_classifier = nn.Linear(config.hidden_size, self.config.num_labels)
        elif config.classifier == 'att+region':
            self.ke_query = nn.Linear(config.hidden_size, 1)
            self.ke_classifier = nn.Sequential(
                nn.Linear(config.hidden_size*2, 128),
                nn.GELU(),
                nn.LayerNorm(128, eps=config.layer_norm_eps),
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, labels=None, 
        ...
        outputs = (logits, outputs[0])  # logits and hidden states
